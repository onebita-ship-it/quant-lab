...
| `simulations/` | 몬테카를로(`monte_carlo`, `_v3`, `_final`, `_tax`, `_v8`, `_v9`, `_worldviews`, `_portfolio`, `_benchmark`) |
| `journal/` | 확정 룰북 기반 매매일지(`daily_check`/`log_trade`/`monthly_report`) |
| `result*.md` | 결과·해석 리포트 (v1=`result.md`, `result_v2~v9.md`, 확정=`result_final.md`, 세후=`result_tax.md`, 연도별=`result_yearly.md`, 국면=`result_regime.md`, 배분=`result_portfolio.md`, 벤치마크=`result_benchmark.md`) |
