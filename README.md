...
| `backtests/` | 실행기(`run_backtest.py`), 지표(`metrics.py`), 버전 비교(`compare_v2~v7.py`), 교차검증(`crossval_v6.py`), 세후+파킹(`tax_parking_backtest.py`), 연도별(`yearly_returns.py`), v8 후보(`candidates_v8`/`run_v8`/`perturb_v8`), 폭락투입(`crash_buyin.py`), v9 2엔진(`engine_v9`/`run_v9`/`perturb_v9`) |
| `simulations/` | 몬테카를로(`monte_carlo`, `_v3`, `_final`, `_tax`, `_v8`, `_v9`) |
| `journal/` | 확정 룰북 기반 매매일지(`daily_check`/`log_trade`/`monthly_report`) |
| `result*.md` | 결과·해석 리포트 (v1=`result.md`, `result_v2~v9.md`, 확정=`result_final.md`, 세후=`result_tax.md`, 연도별=`result_yearly.md`) |
