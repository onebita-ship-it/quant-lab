...
| `backtests/` | 실행기(`run_backtest.py`), 지표(`metrics.py`), 버전 비교(`compare_v2~v7.py`), 실데이터 교차검증(`crossval_v6.py`), 세후+파킹(`tax_parking_backtest.py`) |
| `simulations/` | 몬테카를로(`monte_carlo.py`, `monte_carlo_v3.py`, `monte_carlo_final.py`, `monte_carlo_tax.py`) |
| `journal/` | 확정 룰북 기반 매매일지(`daily_check`/`log_trade`/`monthly_report`) |
| `result*.md` | 결과·해석 리포트 (v1=`result.md`, `result_v2~v7.md`, 확정=`result_final.md`, 세후=`result_tax.md`) |
