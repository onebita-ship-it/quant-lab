...
## 프로젝트 구조
- `data/` — 시세 캐시 (git에 올리지 않음, 스크립트로 재생성. **합성 데이터를 여기 쓰지 말 것** — 실전 판정 도구가 읽는다)
...
- `journal/` — 확정 룰북(§0) 기반 매매일지 CLI (daily_check/log_trade/monthly_report)
- `sheets/` — 구글 시트 일일 기록·판단 시스템 (Apps Script). `signals.gs`는 `journal/common.py`의
  1:1 미러 — **한쪽을 고치면 반드시 `python sheets/parity/run_parity.py` 패리티 감사를 다시 통과시킬 것**
